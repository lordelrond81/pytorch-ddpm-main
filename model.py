...
class DiffusionModel(pl.LightningModule):
    ...
    def get_loss(self, batch, batch_idx):
        """
        Corresponds to Algo 1
        """
        # Get a random time step for each image in the batch - step 3 from Algo 1
        ts = torch.randint(0, self.t_range, [batch.shape[0]], device=self.device)
        noise_imgs = []
        # Generate noise, one for each image in the batch - step 4
        ...
        e_theta = self.forward(noise_imgs, ts)
        # Calculate the loss, ttheta is, the MSE between the predicted noise and the actual noise - or the GD
        loss = nn.functional.mse_loss(
            e_theta.reshape(-1, self.in_size), epsilons.reshape(-1, self.in_size)
        )
        return loss
    ...
    def denoise_sample(self, x, t):
        ...
        with torch.no_grad():
            ...
            e_theta = self.forward(x, t.view(1).repeat(x.shape[0]))
            # Perform the denoising step to take the image from t to t-1
            pre_scale = 1 / math.sqrt(self.alpha(t))
            e_scale = (1 - self.alpha(t)) / math.sqrt(1 - self.alpha_bar(t))
            post_sigma = math.sqrt(self.beta(t)) * z
            x = pre_scale * (x - e_scale * e_theta) + post_sigma
            return x
